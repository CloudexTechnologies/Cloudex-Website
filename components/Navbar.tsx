...
function NavDropdown({ open, onEnter, onLeave }: { open: boolean; onEnter: () => void; onLeave: () => void }) {
  return (
    <div
    ...
    >
      ...
      >
        {NAV_MENU.map((col, ci) => (
          <div key={ci}>
            <div style={{ fontSize: 10, fontWeight: 700, color: "var(--text-3)", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 10, fontFamily: "var(--font-heading)" }}>
              {col.title}
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: 1 }}>
              {col.items.map((item, ii) => <DropdownItem key={ii} {...item} />)}
            </div>
          </div>
        ))}
      </div>
...
function DropdownItem({ label, href, desc }: { label: string; href: string; desc: string }) {
  ...
  return (
    <Link
      ...
      style={{ padding: "8px 10px", borderRadius: "var(--radius-sm)", background: hovered ? "var(--surface-hover)" : "transparent", transition: "background 0.18s", display: "block", textDecoration: "none" }}
    >
      <div style={{ fontSize: 13, fontWeight: 400, color: hovered ? "var(--text-1)" : "var(--text-2)", transition: "color 0.18s", marginBottom: 2 }}>{label}</div>
...
export function Navbar() {
  ...
  const scheduleClose = () => { closeTimer.current = setTimeout(() => setDropdownOpen(false), 250); };

  const isActive = (href: string) => {
    if (href === "/") return pathname === "/";
    if (href === "#") return (pathname.startsWith("/capabilities") && !pathname.startsWith("/capabilities/ai-employees")) || pathname.startsWith("/industries");
    ...
    <>
      ...
      >
        ...
        >
          ...
          {/* ── Nav pill — center, fits only the nav links ── */}
          <div
            style={{
              pointerEvents: "auto",
              position: "relative",
              display: "flex", alignItems: "center",
              padding: scrolled ? "5px 8px" : "8px",
          ...
          >
            <ul
              ...
              style={{ display: "flex", alignItems: "center", gap: 2, listStyle: "none", margin: 0, padding: 0, fontFamily: "var(--font-heading)", fontSize: 14, fontWeight: 500 }}
            >
              {links.map((link, i) => (
                <li
                  key={i}
                  ...
                  {link.hasDropdown ? (
                    <button
                      ...
                      style={{
                        ...
                        transition: "all 0.25s", cursor: "pointer", whiteSpace: "nowrap",
                        border: "none", fontFamily: "var(--font-heading)", fontSize: 14,
                        fontWeight: isActive(link.href) ? 600 : 500,
                        display: "flex", alignItems: "center", gap: 4,
                      }}
                    >
                      {link.label}
                      <svg width="10" height="10" viewBox="0 0 10 10" fill="none" style={{ transition: "transform 0.25s", transform: dropdownOpen ? "rotate(180deg)" : "rotate(0)" }}>
                    ...
                    <Link
                      ...
                      style={{
                        ...
                        fontWeight: isActive(link.href) ? 600 : 500, textDecoration: "none",
                      }}
                    >
                      {link.label}
                      {link.flagship && (
            ...
            {/* Mobile hamburger (inside pill area) */}
            <button
        ...
        {mobileOpen && (
          ...
          >
            {links.filter((l) => !l.hasDropdown).map((link, i) => (
              <Link
                key={i} href={link.href} onClick={() => setMobileOpen(false)}
                style={{ display: "flex", alignItems: "center", gap: 6, padding: "12px 16px", fontSize: 15, fontWeight: 500, color: "var(--text-1)", fontFamily: "var(--font-heading)", borderRadius: "var(--radius-sm)", textDecoration: "none" }}
              >
                {link.label}
                {link.flagship && (
                  <span style={{ fontSize: 9, background: "var(--accent)", color: "#fff", padding: "2px 5px", borderRadius: 4, fontWeight: 700, letterSpacing: "0.05em" }}>FLAGSHIP</span>
                )}
              </Link>
            ))}
